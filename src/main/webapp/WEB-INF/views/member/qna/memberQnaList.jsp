...
<body>
	...
	<div style="text-align: center;">
		...
		<div class="box">
		<form id="sort" name="buySearch" method="post" action="/qna/qnaList">
			<select id="searchType" name="searchType">
                            <option value="">검색조건</option>
                            <option value="c" <c:if test="${pageMaker.cri.searchType eq 'c'}"> selected </c:if>>제목</option>
                            <option value="w" <c:if test="${pageMaker.cri.searchType eq 'w'}"> selected </c:if>>작성자</option>
                            <option value="tc" <c:if test="${pageMaker.cri.searchType eq 'tc'}"> selected </c:if>></option>
            </select>&nbsp;
			...
			<input type="button" id="button" class="btn waves-effect waves-light btn-primary btn-outline-primary" value="전체보기" onClick="location.href='/member/mem/memBuyList';">&nbsp;
		</form>
			<table class="table table-hover table-sm">
            	...
            	<c:forEach var="qna" items="${qnaList }">
                  ...
                  <!-- <tr style="text-align: center; color: #555555;" onClick="location.href='/member/qnaDetail/${qna.q_id }'"> -->
	...
	<div class="row">
				...
				<ul class="btn-group pagination">
					<c:if test="${pageMaker.prev }">
						<li><a href='<c:url value="/member/qnaList${pageMaker.makeQuery(pageMaker.startPage-1)}"/>'>
							<span style="font-weight: bold;">&nbsp;[이전]&nbsp;</span></a></li><span class="col-md-1"></span>
					</c:if>
					<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
						<c:if test="${pageNum eq pageMaker.cri.page}"><li><a href='<c:url value="/member/qnaList${pageMaker.makeQuery(pageNum)}"/>'>
						...
						<c:if test="${pageNum ne pageMaker.cri.page}"><li><a href='<c:url value="/member/qnaList${pageMaker.makeQuery(pageNum)}"/>'>
						<span>&nbsp;${pageNum}&nbsp;</span></a></li><span class="col-md-1">   </span></c:if>
						
					</c:forEach>
					<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
						<li><a href='<c:url value="/member/qnaList${pageMaker.makeQuery(pageMaker.endPage+1)}"/>'>
